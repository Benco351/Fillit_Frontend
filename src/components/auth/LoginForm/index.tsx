  ...
  const onSubmit = async (form: any): Promise<void> => {
    ...
    try {
      ...
      if (mode === 'login') {
        ...
        // .isSignedIn === true when Cognito finished SRP (PASSWORD_VERIFIER handled)
        if (!result.isSignedIn) {
          throw new Error(`Unexpected next step: ${result.nextStep.signInStep}`);
        }

        // obtain JWT (optional: call your backend)
        const session = await fetchAuthSession();
        const token = session.tokens?.accessToken;
        if (!token) throw new Error('Unable to retrieve access token');

        // await instance.post('/auth/check', {}, { headers: { Authorization: `Bearer ${token}` } });
